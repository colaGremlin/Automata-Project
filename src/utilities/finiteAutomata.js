...
export const FiniteAutomata = class {
  ...
  #parseParan(stack) {
    while (stack.includes("(") || stack.includes(")")) {
      // Find the last '(' — this is the innermost opening paren
      let openIndex = -1;
      ...
      // Find the first ')' after it — this is its matching close
      let closeIndex = -1;
      for (let j = openIndex + 1; j < stack.length; j++) {
        if (stack[j] === ")") {
          closeIndex = j;
          break;
        }
      ...
      const subExpression = stack.slice(openIndex + 1, closeIndex);
      const subNFA = this.parseRegularExpression(subExpression);
      stack.splice(openIndex, closeIndex - openIndex + 1, subNFA);
    }
  }
